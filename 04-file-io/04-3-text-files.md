...
- 텍스트 모드의 줄바꿈 변환과 원문 보존 요구를 구분합니다.
{% endhint %}

## 선행 지식

04-1의 경로 검증과 04-2의 파일 조작 정책을 이해해야 합니다.
...
텍스트 모드는 기본적으로 운영체제의 줄바꿈 차이를 처리합니다. 일반적인 줄 단위 처리에서는 기본 설정을 사용하고, CSV처럼 형식 모듈이 줄바꿈을 직접 해석해야 할 때는 `newline=""`을 사용합니다.

```python
with Path("mixed-lines.txt").open(
    "r",
    encoding="utf-8",
    newline=None,
) as file:
...
`newline=None`에서는 `\n`, `\r\n`, `\r`을 읽을 때 `\n`으로 정규화합니다. 원본 바이트의 줄바꿈을 그대로 조사해야 한다면 바이너리 모드로 읽습니다.
...
5. `\n`과 `\r\n`이 섞인 파일을 텍스트 모드와 바이너리 모드에서 각각 확인합니다.
{% endhint %}
...
- [ ] 줄바꿈 정규화가 필요한 경우와 원본 bytes가 필요한 경우를 구분할 수 있습니다.
