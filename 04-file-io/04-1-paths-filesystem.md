...
파일 입출력 오류의 상당수는 파일 내용이 아니라 **프로그램이 어느 위치를 기준으로 어떤 대상을 가리켰는지**에서 시작합니다. 이 절에서는 문자열 경로 대신 `pathlib.Path`를 사용하고, 현재 작업 디렉터리·스크립트 위치·상대 경로·절대 경로를 구분합니다. 마지막에는 사용자 입력 경로가 허용된 실습 디렉터리 밖으로 나가지 못하도록 검증합니다.

{% hint style="info" %}
## 🧭 학습 목표

- 현재 작업 디렉터리와 Python 파일의 위치를 구분합니다.
- 상대 경로와 절대 경로가 해석되는 기준을 설명합니다.
- `Path`로 경로를 구성하고 이름·확장자·상위 경로를 조회합니다.
- 파일·디렉터리·심볼릭 링크를 구분합니다.
- 탐색 결과를 안정된 순서로 필터링합니다.
- `stat()`으로 파일 크기와 수정 시각을 확인합니다.
- 사용자 입력 경로를 허용된 기준 디렉터리 안으로 제한합니다.
- 정상·오류·경계 입력으로 경로 함수를 검증합니다.
{% endhint %}

## 선행 지식

- 변수, 문자열, 리스트, 딕셔너리를 이해해야 합니다.
- 조건문과 반복문을 작성할 수 있어야 합니다.
- 함수 인자·반환값과 `ValueError`, `FileNotFoundError`를 이해해야 합니다.
- 모듈을 `import`하고 `try`/`except`를 사용할 수 있어야 합니다.
...
경로 문자열을 받자마자 파일을 열지 않고 위 질문을 순서대로 확인합니다.

## 1. Path 객체와 실제 파일은 다르다

`Path` 객체를 만드는 동작은 경로를 표현할 뿐 파일이나 디렉터리를 생성하지 않습니다.
...
`Path` 객체가 만들어졌다는 사실은 파일 존재나 접근 권한을 보장하지 않습니다.

## 2. 현재 작업 디렉터리

상대 경로는 Python 파일이 저장된 위치가 아니라 **프로그램을 실행한 현재 작업 디렉터리**를 기준으로 해석됩니다.
...
같은 프로그램도 실행 위치가 달라지면 상대 경로가 다른 대상을 가리킬 수 있습니다.
...
위 실행에서는 `Path("data/users.txt")`가 `project/data/users.txt`를 가리킵니다. 그러나 상위 디렉터리에서 `python project/app.py`로 실행하면 같은 상대 경로가 상위 디렉터리의 `data/users.txt`를 찾습니다.

{% hint style="warning" %}
파일을 찾지 못할 때 경로 문자열을 계속 수정하기 전에 `Path.cwd()`와 실행 명령을 먼저 확인합니다.
{% endhint %}

## 3. 스크립트 위치와 작업 디렉터리

일반 `.py` 파일에서는 `__file__`로 현재 Python 파일의 위치를 구할 수 있습니다.
...
Jupyter Notebook과 일부 대화형 환경에는 `__file__`이 없습니다. 이 환경에서는 작업 디렉터리나 명시적인 프로젝트 기준 경로를 사용합니다.
...
운영체제마다 절대 경로 표현이 다르므로 `/tmp/...`나 `C:\\...` 같은 경로를 학습 코드에 고정하지 않습니다. `Path.cwd()`, `Path.home()` 또는 프로그램 설정값으로 기준 경로를 정합니다.

절대 경로를 다른 기준 경로와 결합할 때는 특히 주의합니다.
...
오른쪽 값이 현재 운영체제에서 절대 경로이면 앞의 `base`가 무시될 수 있습니다. 사용자 입력이 절대 경로인지 먼저 검사해야 합니다.
...
`suffix`는 마지막 확장자 하나만 반환합니다. `.tar.gz`처럼 복합 확장자가 중요하면 `suffixes`를 확인합니다.

이름을 바꾼 새 경로도 만들 수 있습니다.
...
이 메서드는 새 `Path`를 반환할 뿐 실제 파일 이름을 변경하지 않습니다. 파일 이동과 이름 변경은 04-2에서 다룹니다.
...
`resolve()`는 절대 경로를 만들고 `.`·`..`과 확인 가능한 심볼릭 링크를 반영합니다.
...
`resolve()`는 경로를 정리하는 기능입니다. 정리된 경로가 프로그램이 허용한 범위 안에 있는지는 별도로 확인해야 합니다.
...
심볼릭 링크는 다른 파일이나 디렉터리를 가리킬 수 있습니다. `is_file()`과 `is_dir()`은 링크가 가리키는 대상을 따라가므로, 링크 자체를 구분해야 하는 정책이라면 `is_symlink()`를 먼저 확인합니다.
...
존재 확인 직후에도 다른 프로그램이 대상을 삭제하거나 바꿀 수 있습니다. `exists()`만 믿지 말고 실제 파일 작업에서 발생하는 예외도 처리합니다.
...
`exist_ok=True`여도 같은 경로에 일반 파일이 있거나 권한이 부족하면 예외가 발생합니다.
...
`iterdir()`는 한 단계 아래의 모든 항목을 반환합니다.
...
파일시스템의 기본 탐색 순서는 환경마다 다를 수 있습니다. 보고서와 테스트에서 순서가 중요하면 `sorted()`를 적용합니다.
...
glob 패턴은 정규표현식이 아닙니다. 대소문자 구분 방식도 파일시스템에 따라 다를 수 있으므로 결과를 다시 검증합니다. 범위가 넓은 `rglob("*")`은 불필요한 파일과 많은 디렉터리를 탐색할 수 있습니다.
...
`st_size`는 바이트 단위입니다. 텍스트의 문자 수와 같다고 가정하면 안 됩니다. `st_mtime`은 수정 시각을 나타내지만 분석 중 파일이 바뀔 수 있으므로 해시나 내용 분석 결과와 항상 같은 시점의 상태라고 보장되지는 않습니다.

## 11. 허용된 기준 디렉터리 검증

사용자 입력을 파일 경로로 받을 때는 빈 입력과 절대 경로를 거부하고, 정리된 결과가 허용된 기준 디렉터리 안에 있는지 확인합니다.
...
기존 입력 파일은 `must_exist=True`로 확인합니다.
...
`/srv/lab-other`도 문자열상 `/srv/lab`으로 시작할 수 있고 운영체제별 대소문자·구분자 문제도 있습니다. 경로 구성요소를 이해하는 `is_relative_to()`를 사용합니다.
...
이 검사는 기본적인 작업 범위 제한입니다. 검사 뒤 실제 사용 전에 링크나 디렉터리가 교체되는 경쟁 조건까지 완전히 해결하지는 않습니다. 운영 환경에서는 최소 권한, 컨테이너·샌드박스, 운영체제 수준의 안전한 파일 열기 정책을 함께 사용합니다.

## 12. 경로 오류 진단 순서

파일을 찾지 못하거나 잘못된 파일을 열었을 때 다음 순서로 확인합니다.

1. `Path.cwd()`로 현재 작업 디렉터리를 출력합니다.
2. 원래 입력이 상대 경로인지 절대 경로인지 확인합니다.
3. `resolve(strict=False)` 결과를 출력합니다.
4. 존재해야 한다면 `resolve(strict=True)`로 검증합니다.
5. `is_file()`, `is_dir()`, `is_symlink()`로 종류를 확인합니다.
6. 사용자 입력이면 허용된 기준 디렉터리 안인지 확인합니다.
7. 실제 작업의 `FileNotFoundError`, `PermissionError`를 구분합니다.

오류 메시지에는 원본 비밀번호·토큰 같은 민감정보를 포함하지 않습니다.
...
아닙니다. `Path`는 경로를 표현합니다. 파일 생성은 `open()`, `touch()`, `write_text()` 같은 별도 작업이 필요합니다.
...
아닙니다. 상대 경로는 현재 작업 디렉터리를 기준으로 해석됩니다.
...
아닙니다. 정리된 결과가 허용된 기준 디렉터리 안에 있는지 별도로 확인해야 합니다.
...
아닙니다. 존재하지 않거나, 디렉터리·링크·특수 파일 등 다른 대상일 수 있습니다. 필요한 판정을 각각 수행합니다.
...
교사가 제공한 `file-lab/input` 디렉터리에서 `.txt` 파일만 찾아 다음 정보를 리스트로 반환합니다.
...
- 기준 디렉터리가 없으면 `FileNotFoundError`를 발생시킵니다.
- 빈 사용자 입력과 절대 경로를 거부합니다.
- 기준 디렉터리 밖으로 나가는 경로를 거부합니다.
- 디렉터리와 심볼릭 링크를 결과에서 제외합니다.
- `.txt` 파일만 포함합니다.
- 결과는 상대 경로 문자열을 기준으로 정렬합니다.
- 원본 파일을 만들거나 수정하거나 삭제하지 않습니다.
...
1. 정상 상대 경로 한 건을 절대 경로로 변환합니다.
2. 빈 값과 절대 경로를 거부합니다.
3. `..` 이탈 입력을 거부합니다.
4. 존재하는 디렉터리인지 확인합니다.
5. `.txt` 일반 파일만 필터링합니다.
6. 기준 디렉터리에 대한 상대 경로를 저장합니다.
7. 결과를 정렬하고 경계 입력을 검증합니다.
...
- [ ] `Path` 객체 생성과 실제 파일 생성을 구분할 수 있습니다.
- [ ] 현재 작업 디렉터리와 스크립트 디렉터리의 차이를 설명할 수 있습니다.
- [ ] 상대·절대 경로를 판별하고 목적에 맞는 기준 경로를 선택할 수 있습니다.
- [ ] `name`, `stem`, `suffix`, `suffixes`, `parent`, `parts`를 사용할 수 있습니다.
- [ ] `resolve(strict=False)`와 `resolve(strict=True)`를 구분할 수 있습니다.
- [ ] 파일·디렉터리·심볼릭 링크를 구분할 수 있습니다.
- [ ] `iterdir()`, `glob()`, `rglob()`의 탐색 범위를 설명할 수 있습니다.
- [ ] 파일 크기와 수정 시각을 메타데이터로 조회할 수 있습니다.
- [ ] 빈 값·절대 경로·기준 경로 이탈을 거부할 수 있습니다.
- [ ] 문자열 접두 비교 대신 `is_relative_to()`를 사용할 수 있습니다.
- [ ] 정상·오류·경계 입력으로 경로 함수를 검증할 수 있습니다.
- [ ] 원본 파일을 변경하지 않고 파일 목록 보고서를 만들 수 있습니다.
