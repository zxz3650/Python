...
종료 코드의 의미는 프로그램별 계약입니다. 이 챕터의 두 프로젝트는 다음처럼 해석합니다.

| 종료 코드 | 13-6 파일 무결성 변경 탐지기 | 13-7 보안 점검 결과 보고서 |
| --- | --- | --- |
| 0 | 기준선 생성 성공 또는 변경 없음 | 보고서 생성·검증 성공, 입력 오류 없음 |
| 1 | 경로·기준선·읽기·출력 오류 | 헤더·읽기·저장·출력 검증 실패 |
| 2 | 파일 변경 발견 | 보고서 생성·검증 성공, 입력 오류 포함 |

13-6의 코드 2는 승인된 변경인지 검토할 신호입니다. 13-7의 코드 2는 오류 행을 수정할 신호이며, 코드 0이어도 보안상 미흡 항목은 있을 수 있습니다. 종료 코드만으로 보안 상태를 판단하지 않고 변경 목록과 미흡 건수를 함께 읽습니다.

예약 실행은 13-6의 `check` 명령을 사용합니다. 기준선을 매번 새로 만들면 이전 상태와의 비교가 사라집니다. 두 프로젝트 모두 기존 출력을 덮어쓰지 않으므로 검사 시각이나 실행 ID를 포함한 새 출력 경로를 지정합니다.
...
다음: [13-6. 프로젝트 A - 파일 무결성 변경 탐지기](13-6-safe-file-organizer-project.md)
