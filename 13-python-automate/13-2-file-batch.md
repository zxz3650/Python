...
├── intake/
│   ├── manifest.json     입력 파일 목록과 파일별 읽기 설정
│   └── exports/          분석에 사용할 CSV 사본
...
## 2. manifest.json에 입력 파일과 열 이름 기록하기

`manifest.json`은 분석할 파일 목록과 읽기 설정을 담은 JSON 파일입니다. 파일마다 수집한 호스트, 사용한 파서, 시각이 담긴 열 등을 적습니다. 아래의 `columns`는 분석 프로그램의 공통 필드 이름과 원본 CSV의 열 이름을 연결합니다. 이를 **열 매핑**이라고 합니다.
...
오른쪽 열 이름은 **매핑 방법을 설명하기 위한 예**입니다. 특정 EvtxECmd 버전의 실제 헤더라고 가정하지 않습니다. 실제 CSV 헤더를 확인해 바꾸어야 하며, 계정과 IP 등 여러 값이 한 열에 들어 있다면, 필요한 값을 각각 꺼내는 변환 코드를 먼저 작성해야 합니다.

`host`는 수집 자료의 원래 호스트입니다. 분석을 수행하는 PC 이름으로 채우지 않습니다. 도메인 계정·SID와 호스트별 로컬 계정은 구분하고, 별칭을 자동으로 같은 사용자로 합치지 않습니다.

## 3. 분석 결과에 원본 파일과 레코드 위치 남기기

예제는 CSV 파일 내용의 SHA-256 해시, 파일 상대 경로, 헤더를 1로 센 레코드 번호를 정규화 행마다 저장합니다. 줄바꿈이 포함된 CSV 셀은 여러 줄을 차지할 수 있으므로 레코드 번호는 물리적 줄 번호와 다릅니다.
...
| `parse_failed` | 인코딩·CSV 형식·필수 열 누락 등 헤더 오류 |
| `missing` | manifest에 적은 파일을 받지 못함 |

manifest에 적지 않은 자료의 수집 여부는 알 수 없습니다. 기본 프로그램이 디스크 전체를 검색하거나 필요한 자료를 자동으로 찾아내는 것은 아닙니다.
...
- [ ] 파일마다 호스트, 파서와 열 매핑을 기록했습니다.
- [ ] 입력 CSV의 해시와 원본 아티팩트의 수집·인계 기록을 구분할 수 있습니다.
