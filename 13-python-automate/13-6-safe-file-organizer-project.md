...
Python 표준 라이브러리만 사용합니다. 제공 자료는 실제 KAPE·PECmd·EvtxECmd 출력이 아니며 **열 매핑과 오류 처리를 학습하기 위한 실습용 CSV**입니다. 해당 도구들의 원본 CSV와 직접 호환된다고 가정하지 않습니다.
...
manifest에는 `amcache.csv`도 적혀 있지만 파일은 만들지 않습니다. 미수집 상태를 보고하는 연습입니다. 합성 IP `192.0.2.50`은 자료 안의 값이며 접속 대상이 아닙니다.
...
| CSV 바이트·레코드 번호 | 해시·출처 위치·같은 입력에서 동일한 레코드 식별자(UID) 생성 |
...
종료 코드는 `2`이며 처리 상태는 `partial`입니다. 보고서는 저장되었지만, 일부 입력에 오류가 있거나 파일이 누락되었다는 뜻입니다.

| 대조 항목 | 예상값 | 의미 |
| --- | --- | --- |
| 입력 목록의 자료 | 7개 | CSV 6개 + 누락 Amcache 1개 |
| 읽은 CSV 행 | 15개 | 헤더 제외 |
...
자료 파일 형식 전체가 손상된 경우에는 읽지 못한 뒷부분의 행 수까지 알 수 없습니다. 이때는 상태를 `parse_failed`로 기록하고, 오류가 나기 전까지 읽은 행 수만 남깁니다.
...
├── report.json          처리 범위·오류·검토 항목·같은 경로가 기록된 자료 목록
...
| 함수 | 구현할 기능 |
| --- | --- |
| `input_path()` | 입력 폴더 범위·상대 경로·심볼릭 링크 허용 여부 확인 |
...
4. 같은 입력 파일을 두 번 등록하면 오류로 중단되는지 확인합니다.
...
5. 파서·규칙 업데이트 시 열 이름, 자료형과 분석 결과를 다시 확인합니다.
