...
> 실습 준비: [이 절의 노트북 보기](../notebooks/03-4-loops.ipynb) · [03장 교육자료 ZIP 받기](https://github.com/zxz3650/Python/raw/refs/heads/master/downloads/chapter-03.zip) · [자료 준비·실행 안내](../PRACTICE.md#download)
>
...
- iterator의 일회성 소비, 조기 종료, 자료구조 선택의 절충점을 실제 처리 흐름에 적용한다.
{% endhint %}
...
![iterator는 위치를 기억하는 상태를 가진 객체다](../assets/03-4-iterator-mechanics.svg)
...
![enumerate는 번호를 붙이고, zip은 나란히 묶는다](../assets/03-4-enumerate-zip.svg)
...
![continue는 다음 반복으로, break는 루프 밖으로, else는 break가 없을 때만](../assets/03-4-break-continue-else.svg)
...
![삭제하면서 순회하면 인덱스가 밀려 항목을 건너뛴다](../assets/03-4-mutate-while-iterate.svg)
...
## 응용 인사이트 모음: 반복문을 처리 전략으로 확장하기

### 응용 인사이트: 반복문은 상태 전이와 종료 증명을 함께 설계한다

자동화 작업은 “성공할 때까지 반복”이라는 말로 시작하기 쉽다. 그러나 성공이 영원히 오지 않을 수 있으므로, 실제 요구사항은 **성공하거나 허용된 시도 횟수를 모두 사용하면 종료**하는 것이다. 이때 상태는 `attempts`와 `connected`, 종료 조건은 최대 시도 횟수와 성공 여부다.
...
while attempts < max_attempts and not connected:
    attempts += 1
    connected = next(responses, False)

print({"attempts": attempts, "connected": connected})
```

횟수가 고정되어 있다면 `for ... in range()`가 더 간단하고, 성공 여부처럼 상태가 종료를 결정한다면 `while`이 자연스럽다. 어느 방식을 쓰든 **매 반복이 종료에 가까워지는 근거**가 있어야 한다. `attempts += 1`을 조건 분기 뒤에 두고 `continue`하면 갱신이 건너뛰어져 종료 증명이 깨질 수 있다.

여기서는 반복 자체가 반드시 끝나는지에 집중한다. 실패 종류에 따라 다시 시도할지를 결정하고 중복 실행 위험을 다루는 방법은 [03-6의 재시도 인사이트](03-6-exceptions.md#응용-인사이트-재시도는-예외를-잡는-기능이-아니라-제한된-복구-알고리즘이다)에서 확장한다.

{% hint style="info" %}
사고 질문: 응답 목록이 비어 있거나 모든 응답이 `False`여도 위 반복은 끝나는가? `max_attempts`를 없애면 운영 환경에서 어떤 자원을 계속 소비할 수 있는가?
{% endhint %}

### 응용 인사이트: 지연 생성 iterator는 전체 적재를 피할 수 있지만 되감기와 재사용을 포기한다

입력이 값을 필요할 때 한 건씩 제공하는 iterable이라면 iterator로 순차 소비해 전체 데이터를 동시에 보관하지 않는 처리로 확장할 수 있다. 반면 iterator는 현재 위치를 가진 **일회성 상태**이므로, 같은 데이터를 두 번 분석하려 하면 두 번째 결과가 비어 있을 수 있다. 다음 예는 작은 리스트로 이 일회성 동작만 확인한다.

```python
event_iterator = iter(["ALLOW", "DENY", "DENY"])

first_pass = list(event_iterator)
second_pass = list(event_iterator)

print(first_pass)   # ['ALLOW', 'DENY', 'DENY']
print(second_pass)  # []
```

여러 번 순회해야 한다면 원본 iterable에서 새 iterator를 만들거나 리스트로 보관한다. 리스트 보관은 재사용과 인덱스 접근이 쉽지만 데이터 크기만큼 메모리를 사용한다. 이미 존재하는 리스트에 `iter()`를 호출한다고 그 리스트의 메모리가 줄어드는 것도 아니다. iterator를 중간에 디버깅 목적으로 `list()`로 바꾸는 순간 모두 소비된다는 점도 주의한다.

{% hint style="warning" %}
iterator를 함수 두 곳에 차례로 전달하면서 두 함수 모두 전체 데이터를 받을 것이라고 가정하지 않는다. 누가 소비하는지, 한 번만 순회하는지 함수 계약에 드러내야 한다.
{% endhint %}

### 응용 인사이트: 조기 종료는 성능 최적화이면서 결과의 의미다

차단 대상이 **하나라도 존재하는지** 확인하는 작업은 첫 일치에서 끝내도 결과가 변하지 않는다. 이 경우 `break`는 불필요한 검사를 줄이고 “첫 일치만 필요하다”는 요구사항을 코드에 표현한다.

```python
events = [
    {"ip": "10.0.0.5", "action": "ALLOW"},
    {"ip": "198.51.100.9", "action": "DENY"},
    {"ip": "203.0.113.10", "action": "DENY"},
]
...
print(first_denied)
print(checked_count)  # 2
```

최선의 경우 첫 항목에서 끝나지만, 일치가 없으면 모든 항목을 확인하므로 최악의 실행량은 여전히 데이터 개수에 비례한다. 모든 DENY 이벤트나 정확한 총건수가 필요하다면 조기 종료하면 안 된다. 즉, `break`의 사용 여부는 속도보다 먼저 **첫 항목·존재 여부·전체 목록 중 무엇을 구하는가**로 결정한다.

### 응용 인사이트: 반복 횟수보다 자료구조 선택이 더 큰 차이를 만들 수 있다

이벤트 `n`개를 차단 목록 `m`개와 리스트 중첩 반복으로 비교하면 최악에는 `n × m`번 비교한다. 차단 목록을 집합으로 준비하면 추가 메모리를 사용하는 대신 각 이벤트의 멤버십 검사를 빠르게 수행할 수 있다.
...
데이터가 몇 개뿐이라면 리스트도 충분하고 변환 비용이 더 클 수 있다. 그러나 같은 차단 목록을 많은 이벤트에 반복 적용한다면 집합 준비 비용을 한 번 지불하는 편이 유리하다. 성능 문제를 무조건 중첩 반복 탓으로 돌리기보다 **입력 크기, 반복 사용 횟수, 필요한 순서, 허용 메모리**를 함께 판단한다.

세트로 변환할 때 순서와 빈도 정보가 사라지는 문제는 [03-2의 자료구조 응용 인사이트](03-2-strings-collections.md#응용-인사이트-편리한-자료구조가-정보를-없앨-수도-있다)를 참고한다. 이 절에서는 같은 목록을 여러 번 조회할 때의 검색 비용에 초점을 둔다.

{% hint style="info" %}
사고 질문: 차단 목록의 입력 순서를 보고서에도 유지해야 한다면 집합만으로 요구사항을 충족할 수 있는가? 빠른 검색용 집합과 표시용 리스트를 따로 둘 때 두 자료의 일관성은 누가 보장해야 하는가?
{% endhint %}
...
- [ ] iterator를 재사용할 수 없는 이유와 리스트로 보관할 때의 비용을 설명한다.
- [ ] 첫 일치와 전체 일치를 구분하고 조기 종료의 최선·최악 실행량을 판단한다.
...
- 값을 지연 생성하는 iterator는 전체 적재를 피하는 순차 처리에 유용하지만 소비 후 되감을 수 없으며, 검색용 자료구조와 조기 종료는 결과 요구사항을 먼저 확인한 뒤 선택한다.
