...
![변수는 상자가 아니라 이름표다](../assets/03-1-variable-reference.svg)
...
![여섯 가지 기본 자료형 지도](../assets/03-1-data-types-map.svg)
...
### 응용 인사이트: 숫자의 정밀도도 업무 규칙이다

위험 점수나 측정값을 `float`로 저장하는 것은 빠르고 편리하지만, **몇 자리까지 같은 값으로 볼지**는 자료형이 대신 정해 주지 않는다. 예를 들어 임계값이 정확한 소수 계약인지, 센서가 만든 근삿값인지에 따라 판단 방법이 달라진다.

```python
calculated_score = 0.1 + 0.2

print(calculated_score)         # 0.30000000000000004
print(calculated_score == 0.3)  # False
print(round(calculated_score, 2) == 0.3)  # True
```

`round()`는 표시 자릿수나 판정 정밀도가 명시된 경우에만 적용한다. 모든 값에 임의로 반올림하면 임계값 바로 주변의 데이터를 서로 다른 기준으로 분류할 수 있다. 금액처럼 십진수 자체가 계약인 데이터는 `Decimal`, 측정 오차를 포함한 값은 허용 오차나 반올림 자리와 같은 별도 정책을 검토한다.

> 사고 질문: 위험 점수 `0.299`, `0.300`, `0.301` 중 어느 값을 임계값 `0.30` 이상으로 볼 것인가? 코드보다 먼저 데이터 제공자의 정밀도 정의를 확인해야 하는 이유는 무엇인가?
...
### 응용 인사이트: 자료형은 구성 요소 사이의 데이터 계약이다

명령줄, 로그, 환경 변수처럼 프로그램 바깥에서 들어온 값은 흔히 문자열이다. 분석 코드 안에서 포트를 계속 문자열로 두면 수치 비교가 아니라 문자 순서 비교가 일어날 수 있다.

```python
raw_port = "80"
port = int(raw_port)

print(raw_port > "443")  # True: 첫 문자 "8"과 "4"를 비교
print(port > 443)         # False: 숫자 80과 443을 비교
```

따라서 입력 경계에서는 원본과 해석된 값을 분리하고, 이후 코드에는 의미가 확정된 자료형을 전달하는 편이 안전하다. 원본을 남기면 오류를 재현하고 입력 출처를 설명하기 쉽지만 저장 공간과 관리할 값이 늘어난다. 반대로 변환 결과만 남기면 간단하지만 앞자리 `0`, 공백과 같은 원래 표현을 잃는다.
...
print(repr(raw_port))  # ' 0443 '
print(port)            # 443
```

> 사고 질문: 보고서에는 정수 `443`만 필요하더라도 감사 기록에는 원문 `" 0443 "`을 남겨야 하는 상황이 있는가?
...
### 응용 인사이트: 결측값과 정상적인 0을 분리한다

집계 결과 `0`은 “확인했지만 한 건도 없음”이고 `None`은 “아직 집계하지 않았거나 값을 받지 못함”일 수 있다. 두 상태를 하나로 합치면 데이터 수집 실패를 정상 상태로 오해할 수 있다.

```python
failed_count = 0
previous_failed_count = None

print(failed_count == 0)               # True: 집계 완료, 0건
print(previous_failed_count is None)   # True: 아직 확인되지 않음
```

`"UNKNOWN"` 같은 문자열을 숫자 변수에 섞는 방법도 가능해 보이지만, 이후 덧셈과 범위 비교가 어려워진다. 숫자 또는 `None`이라는 일관된 표현을 선택하면 사용하는 쪽에서 결측 여부를 명시적으로 처리해야 한다는 비용이 생기는 대신, 잘못된 계산을 일찍 발견할 수 있다.

> 사고 질문: 하루 평균 실패 횟수를 구할 때 `None`을 `0`으로 바꾸면 어떤 통계 왜곡이 생길 수 있는가?
...
![형식 검증과 범위 검증은 서로 다른 질문이다](../assets/03-1-validation-flow.svg)
...
### 응용 인사이트: 정규화·변환·검증은 서로 다른 단계다

외부 입력을 처리할 때는 보통 **표현을 정리하고 → 자료형을 변환하고 → 업무 규칙을 검증하는** 순서를 사용한다. 각 단계가 답하는 질문이 다르기 때문이다.

```python
raw_port = " 443 "                 # 수집한 원문
normalized_port = raw_port.strip() # 허용한 표현 차이만 정리
port = int(normalized_port)        # 정수로 해석 가능한지 확인
is_valid_port = 1 <= port <= 65535 # 포트로 허용되는지 확인

print(port, is_valid_port)         # 443 True
```

공백 제거는 앞뒤 공백을 허용한다는 입력 규칙이 있을 때 합리적이다. 그러나 `"443/tcp"`에서 숫자만 억지로 뽑아 `443`으로 만드는 처리는 잘못된 입력을 정상처럼 보이게 할 수 있다. 엄격하게 거부하면 데이터 손실을 빨리 발견하지만, 실제로 허용해야 할 표현 차이까지 거부할 수 있다. 무엇을 정리하고 무엇을 오류로 남길지 입력 계약으로 정해야 한다.

> 사고 질문: `"00443"`, `"443 "`, `"443/tcp"` 가운데 어느 표현까지 같은 포트로 받아들일 것인가? 그 기준을 입력 생산자와 합의하지 않으면 어떤 문제가 생기는가?
...
![재대입해도 기존 객체는 바뀌지 않는다](../assets/03-1-reassignment.svg)
...
- [ ] 원본·정규화값·변환값을 분리하고 각 값의 보존 이유를 설명할 수 있다.
- [ ] `float` 정밀도와 `None` 처리 기준이 업무 결과에 미치는 영향을 설명할 수 있다.
...
- 자료형은 구성 요소 사이의 데이터 계약이며, 원본을 보존하면 변환 결과를 추적할 수 있다.
- 정규화·변환·검증의 허용 범위와 수치 정밀도는 업무 규칙으로 명시한다.
